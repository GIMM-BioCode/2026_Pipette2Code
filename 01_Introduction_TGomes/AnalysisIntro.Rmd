...
## you will also need to install presto for faster DE
#devtools::install_github('immunogenomics/presto')
## and glmGamPoi for faster SCT
...
```{r}
srat = readRDS("/mnt/nfs/EMBO/IntroAnalysis/dataset1.RDS")
```
...
And as scatterplots. This works on small sampples because there are not that many points, but for larger datasets other alternatives (e.g. 2D hexbins) may be more efficient.  
...
```{r}
ncomp = CHOOSE_RESOLUTION # an integer
```
...
How does the metadata look now?
...
# alternative built in Seurat 
## this will not performa test for all genes, only those with a FC above the threshold
...
mk_clusters |>
  ...
  top_n(n = 3, wt = avg_log2FC) # top genes per cluster, ranked by logFC
...
```{r}
mk_comp = FindMarkers(srat_filt, ident.1 = "1", ident.2 = "2", assay = "SCT", 
...
```{r}
srat_filt = FindSubCluster(srat_filt, cluster = "2", graph.name = paste0(red, ncomp))
...
degenes = mk_clusters %>%
  filter(cluster=="1" & p_val_adj<=0.05 & avg_log2FC>0.2) %>%
  arrange(-avg_log2FC)
allgenes = rownames(srat_filt) # all genes, to serve as background

gprof_enr = gost(degenes$gene, ordered_query = T, 
...
topmk = mk_clusters |>
  group_by(cluster) |> # for each cluster
  filter(p_val_adj<=0.05) |> # only p-value below 0.05
  top_n(n = 10, wt = avg_log2FC) # top genes per cluster, ranked by logFC
